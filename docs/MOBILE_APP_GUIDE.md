...
End-to-end walkthrough for building an Android Solana app on Artemis 2.3.2. The guide assumes you already know Kotlin, Coroutines, and Compose. Every code sample maps to a real API in the repo with a file path you can click into.
...
dependencies {
    ...
    implementation("xyz.selenus:artemis-seed-vault:2.3.2") // Saga only
    ...
    implementation("xyz.selenus:artemis-metaplex:2.3.2")
}
```

The current published version is `2.3.2`. The `version` field in [../gradle.properties](../gradle.properties) is the source of truth.
