 ...
 * 5. See DEVNET_TESTING_GUIDE.md for devnet testing
 */
...
println("[OK] Keypair generation works")
println("[OK] Pubkey: ${pubkey.toBase58()}")
...
println("[OK] RPC Client initialized: $devnetEndpoint")
...
// System Program transfer: 4 bytes LE instruction index (2) + 8 bytes LE lamports
val transferData = java.nio.ByteBuffer.allocate(12)
...
println("[OK] Transaction created with ${tx.instructions.size} instruction(s)")
...
println("[OK] Generated ${keypairs.size} keypairs")
...
println("[OK] Parsed pubkey: ${pubkey.toBase58()}")
...
println("[OK] JSON serialization works")
println(jsonString)
...
println("[OK] OkHttp client initialized")

assertNotNull(httpClient)
}

@Test
fun `9 - SDK Readiness Check`() = runBlocking {
...
println("[FAIL] Keypair: ${e.message}")
}
...
println("[FAIL] TX Builder: ${e.message}")
}
...
val icon = if (result) "[OK]" else "[FAIL]"
println("$icon $check")
}
