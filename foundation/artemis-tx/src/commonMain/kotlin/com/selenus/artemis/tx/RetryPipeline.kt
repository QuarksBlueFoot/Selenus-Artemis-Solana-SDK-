...
object RetryPipeline {
        ...
        for (attempt in 1..policy.maxRetries) {
            try {
                ...
                return RetryResult.Success(
                    result = result,
                    attempts = attempt,
                    totalTimeMs = currentTimeMillis() - startTime,
    ...
    fun classifyError(e: Exception): RetryError {
        ...
        return when {
            // Network errors (check by message pattern for KMP compatibility)
            message.contains("timeout", ignoreCase = true) ||
            message.contains("connect", ignoreCase = true) && message.contains("refused", ignoreCase = true) ||
