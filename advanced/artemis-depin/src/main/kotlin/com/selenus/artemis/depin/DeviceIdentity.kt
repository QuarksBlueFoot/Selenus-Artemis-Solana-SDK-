...
data class LocationProof(
  ...
  /**
   * Verify this location proof was signed by the claimed device.
   */
  fun verify(): Boolean {
    val message = "$latitude:$longitude:$timestamp"
    return try {
      val signatureBytes = java.util.Base64.getDecoder().decode(signature)
      device.verify(signatureBytes, message.encodeToByteArray())
