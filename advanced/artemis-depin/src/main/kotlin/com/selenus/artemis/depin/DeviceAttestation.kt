...
object DeviceAttestation {
    ...
    fun verifyAttestation(
        ...
        networkId: String = "default",
        maxAgeMs: Long = 300_000 // 5 minute default
    ): Boolean {
        ...
        if (proof.timestamp < now - maxAgeMs) return false // Older than maxAge
        
        if (proof.signature.isEmpty()) return false
        
        // Reconstruct the attestation message
        val message = buildAttestationMessage(
            proof.deviceId,
            proof.timestamp,
            proof.nonce,
            proof.metadata["networkId"] ?: networkId
        )
        
        // Decode the Base64 signature and verify using Ed25519
        return try {
            val signatureBytes = Base64.getDecoder().decode(proof.signature)
            proof.deviceId.verify(signatureBytes, message)
