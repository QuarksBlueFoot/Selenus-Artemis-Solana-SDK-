...
class ShamirSecretSharingTest {
    ...
    fun `split and recover 2-of-3 scheme`() {
        ...
        assertTrue(shares.map { it.index }.distinct().size == 3)
    ...
    fun `recovery with insufficient shares fails`() {
        ...
        assertEquals(3, (result as RecoveryResult.NeedMoreShares).need)
        assertEquals(2, result.have)
    }
    ...
    fun `share serialization roundtrip`() {
        ...
        for (share in shares) {
            val serialized = share.serialize()
            val deserialized = ShamirShare.deserialize(serialized)
            
            assertNotNull(deserialized)
            assertEquals(share.index, deserialized.index)
    ...
    fun `recovery from deserialized shares works`() {
        ...
        val serialized = shares.map { it.serialize() }
        val deserialized = serialized.map { ShamirShare.deserialize(it) }
    ...
    @Test
    fun `invalid serialized data throws`() {
        val invalid = byteArrayOf(0x01, 0x02) // Too short
        
        assertThrows<IllegalArgumentException> {
            ShamirShare.deserialize(invalid)
        }
    }
