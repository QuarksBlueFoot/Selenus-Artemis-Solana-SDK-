...
class StreamingModuleTest {
    ...
    fun `field types have correct sizes`() {
        assertEquals(1, FieldType.U8.defaultSize)
        assertEquals(2, FieldType.U16.defaultSize)
        assertEquals(4, FieldType.U32.defaultSize)
        assertEquals(8, FieldType.U64.defaultSize)
        assertEquals(8, FieldType.I64.defaultSize)
        assertEquals(1, FieldType.BOOL.defaultSize)
        assertEquals(32, FieldType.PUBKEY.defaultSize)
    }
    ...
    fun `buffer pool creates buffers of correct size`() {
        val pool = BufferPool(maxSize = 10, bufferSize = 1024)
    ...
    fun `buffer pool clears all buffers`() {
        ...
        // Pool channel is closed after clear
        assertNotNull(pool)
    }
    ...
    @Test
    fun `subscription config is accessible`() {
        val config = StreamConfig(
            poolSize = 64,
            bufferSize = 4096,
            historySize = 10,
        ...
        assertEquals(64, config.poolSize)
