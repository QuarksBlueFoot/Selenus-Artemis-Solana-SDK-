...
class TransactionBatchEngine(
     ...
     * combine them into transactions, respecting:
     * - Compute unit limits per transaction
     * - Instruction count limits
     * - Account write conflicts between operations
     * 
    ...
    fun plan(
        ...
        var currentWriteAccounts = mutableSetOf<Pubkey>()
        ...
        for (operation in sortedOperations) {
            ...
            // Check for write-write conflicts between operations
            val operationWriteAccounts = getWriteAccounts(operation)
            val hasWriteConflict = config.optimizeOrdering && 
                operationWriteAccounts.any { it in currentWriteAccounts }
            
            if (wouldExceedCompute || wouldExceedInstructions || hasWriteConflict) {
                ...
                currentWriteAccounts = operationWriteAccounts.toMutableSet()
            } else {
                // Add to current batch
                currentOperations.add(operation)
                currentComputeUnits += operation.estimatedComputeUnits
                currentInstructionCount += operationInstructions
                currentWriteAccounts.addAll(operationWriteAccounts)
            }
        }
    ...
    suspend fun execute(
        ...
        for ((index, batch) in plan.batches.withIndex()) {
            ...
            // Build the transaction from batched instructions and delegate to the executor
            val result = try {
                val tx = Transaction()
    ...
    /**
     * Analyze account access conflicts between operations.
     *
     * Returns pairs of operation IDs that have write-write conflicts
     * (both operations write to the same account). Operations with
     * conflicts should be placed in separate batches or carefully ordered.
     */
    fun analyzeConflicts(operations: List<BatchOperation>): List<AccountConflict> {
        val conflicts = mutableListOf<AccountConflict>()
        
        for (i in operations.indices) {
            val writesA = getWriteAccounts(operations[i])
            for (j in i + 1 until operations.size) {
                val writesB = getWriteAccounts(operations[j])
                val conflicting = writesA.intersect(writesB)
                if (conflicting.isNotEmpty()) {
                    conflicts.add(AccountConflict(
                        operationA = operations[i].id,
                        operationB = operations[j].id,
                        conflictingAccounts = conflicting.toList(),
    ...
    private fun getWriteAccounts(operation: BatchOperation): Set<Pubkey> {
        return operation.instructions
            .flatMap { ix -> ix.accounts.filter { it.isWritable }.map { it.pubkey } }
            .toSet()
    }
