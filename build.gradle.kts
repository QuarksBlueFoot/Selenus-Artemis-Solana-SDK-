...
val artemisModuleDescriptions: Map<String, String> = mapOf(
    ...
    "artemis-wallet-mwa-walletlib-android" to "Wallet-role Mobile Wallet Adapter 2.0 library for Android: parses association URIs, runs the wallet end of the HELLO handshake, dispatches authorize / sign / send requests to the wallet's UI.",
...
subprojects {
    ...
    plugins.withId("org.jetbrains.kotlin.multiplatform") {
        ...
        // Maven Central rejects components that don't ship a javadoc jar.
        // KMP target publications (common metadata, jvm, androidRelease,
        // native) don't generate one on their own, so Central Portal drops
        // the deployment with "Javadocs must be provided but not found in
        // entries". Attach an empty, signed javadoc jar — Central accepts
        // that shape; a Dokka jar would be strictly better but adds weight
        // on every KMP module for no signal beyond the sources jar already
        // provides.
        //
        // Critical: each publication needs its OWN jar task with its OWN
        // output file path. A single shared javadocJar produces one output
        // file that every publication's sign task tries to sign, which
        // collides on the resulting `.asc` path and fails Gradle's
        // implicit-dependency validation with a duplicate-sign conflict.
        // Keying by publication name gives each a distinct file.
        configure<PublishingExtension> {
            publications.withType<MavenPublication>().configureEach {
                pomConfig()
                val pubName = name
                val jarTask = tasks.register<Jar>("emptyJavadocJar${pubName.replaceFirstChar { it.uppercase() }}") {
...
// =============================================================================
// API surface drift gate.
//
// Re-runs the dump task into a temp file and compares against the
// committed snapshot. CI invokes this; the build fails when a compat
// module's public surface changes without a matching snapshot update.
// Pair the snapshot bump with the upstream-version pin in each compat
// module's build.gradle.kts (`extra["upstream.version"]`) so reviewers
// can see both moving together.
//
// Run with: ./gradlew verifyApiSnapshots
// =============================================================================
tasks.register("verifyApiSnapshots") {
    group = "verification"
    description = "Fails the build when a committed compat API snapshot drifts from source"
    ...
    doLast {
        val drifted = mutableListOf<Pair<String, String>>()
        compatModules.forEach { mod ->
            rootProject.findProject(":$mod") ?: return@forEach
            val src = file("interop/$mod/src")
            if (!src.exists()) return@forEach
            val expected = file("interop/$mod/api/$mod.api")
            if (!expected.exists()) {
                drifted += mod to "missing snapshot at ${expected.relativeTo(rootDir).invariantSeparatorsPath}; run ./gradlew dumpApi"
                return@forEach
            }
            // Recompute the snapshot in-memory using the same logic as
            // dumpApi. Avoids reading from disk twice.
            val rebuilt = buildString {
                appendLine("# API surface for $mod")
                appendLine("# Generated by ./gradlew dumpApi. Commit this file; CI diffs against it.")
                appendLine()
                src.walkTopDown()
                    .filter { it.isFile && it.name.endsWith(".kt") }
                    .sortedBy { it.relativeTo(src).invariantSeparatorsPath }
                        ...
                        val decls = ktFile.useLines { seq ->
                            seq.mapIndexedNotNull { idx, line ->
                                ...
                                val isDecl = trimmed.startsWith("class ") ||
                                    ...
                                    trimmed.startsWith("var ") ||
                                    trimmed.startsWith("const val ")
                                if (isPublic && isDecl) "  L${idx + 1}  $trimmed" else null
                            }.toList()
                        }
                        if (decls.isNotEmpty()) {
                            appendLine("## $relPath (package $packageLine)")
                            decls.forEach { appendLine(it) }
                            appendLine()
                        }
                    }
            }.trimEnd('\n')
            val committed = expected.readText().trimEnd('\n').replace("\r\n", "\n")
            val rebuiltNorm = rebuilt.replace("\r\n", "\n")
            if (committed != rebuiltNorm) {
                // Compute a minimal diff hint so the CI log shows the
                // delta size without dumping the full file.
                val cl = committed.lines()
                val rl = rebuiltNorm.lines()
                val added = rl.size - cl.size
                drifted += mod to ("snapshot drifted (committed=${cl.size} lines, rebuilt=${rl.size} lines, delta=$added). " +
                    "Run ./gradlew dumpApi and review the diff before committing.")
            }
        }
        if (drifted.isNotEmpty()) {
            val msg = drifted.joinToString("\n") { (m, d) -> "  $m: $d" }
            error("Compat API surface drift detected in ${drifted.size} module(s):\n$msg")
        }
        println("All ${compatModules.size} compat API snapshots match source.")
    }
}
...
tasks.register("checkDependencyRings") {
    ...
    doLast {
        ...
        val mobile = setOf(
            "artemis-wallet", "artemis-wallet-mwa-android", "artemis-seed-vault",
            "artemis-wallet-mwa-walletlib-android", "artemis-compose"
        )
        val ecosystem = setOf(
