...
class AnchorProgram(
    ...
    private fun computeAccountDiscriminator(name: String): ByteArray {
        ...
        val preimage = "account:$name".encodeToByteArray()
    ...
    private fun computeEventDiscriminator(name: String): ByteArray {
        ...
        val preimage = "event:$name".encodeToByteArray()
        val hash = Crypto.sha256(preimage)
        return hash.copyOfRange(0, 8)
    }
...
class AccountTypeFetcher(
    ...
    /**
     * Watch account for changes (reactive polling).
     * Polls the account at the given interval and emits decoded updates when data changes.
     *
     * @param address The account address to watch
     * @param rpcApi RPC client for fetching account data
    ...
    fun watch(
        ...
        while (true) {
            val account = fetch(address, rpcApi)
            if (account != null && (lastData == null || !account.rawData.contentEquals(lastData))) {
                lastData = account.rawData
                emit(account)
            }
            delay(intervalMs)
        }
        ...
        val rpcFilters = buildJsonArray {
            ...
            addJsonObject {
                putJsonObject("memcmp") {
                    put("offset", 0)
                    put("bytes", PlatformBase64.encode(discriminator))
                    put("encoding", "base64")
                }
            }
            ...
            filters.forEach { filter ->
                addJsonObject {
                    putJsonObject("memcmp") {
                        ...
                        put("bytes", PlatformBase64.encode(filter.bytes))
                        put("encoding", "base64")
                    }
                }
            }
...
class PdaBuilder(private val program: AnchorProgram) {
            ...
            val seedBytes = when (seed) {
                is IdlSeed.Const -> {
                    ...
                    when (val value = seed.value) {
                        is JsonPrimitive -> {
                            if (value.isString) {
                                value.content.encodeToByteArray()
                            } else {
                                // Numeric constant
                                val num = value.long
                                ByteArray(8).also { buf -> for (i in 0 until 8) buf[i] = ((num ushr (i * 8)) and 0xFF).toByte() }
                            }
                        }
                        is JsonArray -> {
    ...
    private fun serializeSeedValue(value: Any): ByteArray {
        return when (value) {
            ...
            is Long -> ByteArray(8).also { buf -> for (i in 0 until 8) buf[i] = ((value ushr (i * 8)) and 0xFF).toByte() }
            is Int -> ByteArray(4).also { buf -> for (i in 0 until 4) buf[i] = ((value ushr (i * 8)) and 0xFF).toByte() }
            is ByteArray -> value
