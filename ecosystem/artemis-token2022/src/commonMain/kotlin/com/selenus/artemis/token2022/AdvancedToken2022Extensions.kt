...
object AdvancedToken2022Extensions {
    ...
    fun initializeInterestBearingMint(
        ...
        if (rateAuthority != null) {
            data[offset++] = 1
            rateAuthority.bytes.copyInto(data, destinationOffset = offset)
            offset += 32
        } else {
            data[offset++] = 0
            offset += 32
    ...
    fun initializePermanentDelegate(
        ...
        delegate.bytes.copyInto(data, destinationOffset = 1)
    ...
    fun initializeMetadataPointer(
        ...
        // Metadata address
        metadataAddress.bytes.copyInto(data, destinationOffset = offset)
    ...
    fun updateMetadataPointer(
        ...
        val data = ByteArray(33)
        data[0] = 42
        newMetadataAddress.bytes.copyInto(data, destinationOffset = 1)
    ...
    fun initializeGroupPointer(
        ...
        groupAddress.bytes.copyInto(data, destinationOffset = offset)
    ...
    fun initializeMemberPointer(
        ...
        memberAddress.bytes.copyInto(data, destinationOffset = offset)
    ...
    fun initializeConfidentialTransferMint(
        ...
        if (auditorElgamalPubkey != null) {
            data[offset++] = 1
            auditorElgamalPubkey.copyInto(data, destinationOffset = offset)
        } else {
            data[offset] = 0
        }
    ...
    fun configureConfidentialTransferAccount(
        ...
        decryptableZeroBalance.copyInto(data, destinationOffset = offset, startIndex = 0, endIndex = minOf(64, decryptableZeroBalance.size))
        offset += 64
    ...
    fun initializeMintCloseAuthority(
        ...
        if (closeAuthority != null) {
            data[offset++] = 1
            closeAuthority.bytes.copyInto(data, destinationOffset = offset)
        } else {
            data[offset] = 0
        }
    ...
    /**
     * Validate that a set of extensions is compatible.
     *
     * Certain Token-2022 extensions cannot be combined. This method checks known
     * incompatibilities and returns a list of error messages (empty if valid).
     */
    fun validateExtensions(extensions: List<MintExtension>): List<String> {
        val errors = mutableListOf<String>()
        val types = extensions.map { it::class }

        // NonTransferable + TransferFee makes no sense (can't charge fees on non-transfers)
        if (MintExtension.NonTransferable::class in types && MintExtension.TransferFee::class in types) {
            errors.add("NonTransferable and TransferFee are incompatible: fees cannot apply to non-transferable tokens")
        }

        // NonTransferable + TransferHook is contradictory
        if (MintExtension.NonTransferable::class in types && MintExtension.TransferHook::class in types) {
            errors.add("NonTransferable and TransferHook are incompatible: hooks cannot trigger on non-transferable tokens")
        }

        // Duplicate extension types
        val duplicates = types.groupBy { it }.filter { it.value.size > 1 }
        for ((type, _) in duplicates) {
            errors.add("Duplicate extension: ${type.simpleName}")
        }
     ...
     *
     * Validates extension compatibility before building. Throws if incompatible
    ...
    fun prepareMintWithExtensions(
        ...
        val errors = validateExtensions(extensions)
        require(errors.isEmpty()) { "Incompatible extensions: ${errors.joinToString("; ")}" }
    ...
    private fun initializeTransferFeeConfig(
        ...
        if (transferFeeConfigAuthority != null) {
            data[offset++] = 1
            transferFeeConfigAuthority.bytes.copyInto(data, destinationOffset = offset)
            offset += 32
        } else {
            data[offset++] = 0
            offset += 32
        ...
        if (withdrawWithheldAuthority != null) {
            data[offset++] = 1
            withdrawWithheldAuthority.bytes.copyInto(data, destinationOffset = offset)
            offset += 32
        } else {
            data[offset++] = 0
            offset += 32
    ...
    private fun createAccountInstruction(
        ...
        owner.bytes.copyInto(data, destinationOffset = 20)
