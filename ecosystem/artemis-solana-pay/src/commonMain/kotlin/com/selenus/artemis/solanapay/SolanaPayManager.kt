...
class SolanaPayManager(
    ...
    data class TransactionRequest(
        ...
        fun toUri(): String {
            ...
            sb.append(percentEncode(link))
            val params = mutableListOf<String>()
            label?.let { params.add("label=${percentEncode(it)}") }
            message?.let { params.add("message=${percentEncode(it)}") }
    ...
    private val referenceToSession = mutableMapOf<String, String>()
    ...
    fun parseUri(uri: String): ParsedRequest {
        return try {
            ...
            if (uri.startsWith("solana:http")) {
                val decoded = percentDecode(uri.removePrefix("solana:"))
    ...
    suspend fun verifyPayment(
        ...
        // Check expiration
        if (currentTimeMillis() - session.createdAtMs > config.paymentTimeoutMs) {
    ...
    companion object {
        /**
         * Create a default RPC-based payment verifier.
         * Uses `getSignaturesForAddress` with the reference pubkey to detect transactions
         * that include it as a non-signer account (Solana Pay verification pattern).
         *
         * @param rpcApi The RPC client to query signatures
         * @param commitment Commitment level to check (default "confirmed")
         */
        fun rpcVerifier(
            rpcApi: RpcApi,
            commitment: String = "confirmed"
        ): suspend (Pubkey) -> VerificationResult? = { reference ->
            val signatures = rpcApi.getSignaturesForAddress(
            ...
            if (signatures.isEmpty()) {
                ...
                val entry = signatures.first().jsonObject
                val sig = entry["signature"]?.jsonPrimitive?.content ?: ""
                val slot = entry["slot"]?.jsonPrimitive?.long ?: 0L
...
object SolanaPayQR {
    ...
    fun transactionRequestQR(apiEndpoint: String): String {
        return "solana:${percentEncode(apiEndpoint)}"
    }
