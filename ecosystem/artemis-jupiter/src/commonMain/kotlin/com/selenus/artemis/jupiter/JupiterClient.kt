...
class JupiterClient private constructor(
    ...
    suspend fun getQuote(request: QuoteRequest): QuoteResponse {
        ...
        return executeGet(url) { responseBody ->
    ...
    suspend fun buildSwap(request: SwapRequest): SwapResponse {
        ...
        return executePost(config.swapApiUrl, body) { responseBody ->
    ...
    suspend fun getSwapInstructions(request: SwapRequest): SwapInstructionsResponse {
        ...
        return executePost(config.swapInstructionsApiUrl, body) { responseBody ->
    ...
    suspend fun getProgramIdToLabel(): Map<String, String> {
        return executeGet("${config.baseUrl}/program-id-to-label") { responseBody ->
    ...
    suspend fun getTokenInfo(mint: String): TokenInfo? {
        return try {
            executeGet("${config.tokensApiUrl}/$mint") { responseBody ->
    ...
    suspend fun searchTokens(query: String): List<TokenInfo> {
        return try {
            executeGet("${config.tokensApiUrl}/search?query=$query") { responseBody ->
    ...
    companion object {
         ...
         * Uses platform-specific HTTP client (OkHttp on JVM).
         */
        ...
        /**
         * Create a Jupiter client with a custom HTTP client.
         */
        fun create(httpClient: HttpApiClient, config: JupiterConfig = JupiterConfig()): JupiterClient {
            val json = Json {
                ignoreUnknownKeys = true
                isLenient = true
                encodeDefaults = true
...
data class QuoteResponse(
    ...
    fun getOutputAmountDecimal(decimals: Int): Double {
        return outAmount.toDouble() / pow10(decimals)
    }
    
    /** Human-readable input amount with decimals */
    fun getInputAmountDecimal(decimals: Int): Double {
        return inAmount.toDouble() / pow10(decimals)
    }
    
    /** Get exchange rate (output per input) */
    fun getExchangeRate(inputDecimals: Int, outputDecimals: Int): Double {
        val inAmt = getInputAmountDecimal(inputDecimals)
        val outAmt = getOutputAmountDecimal(outputDecimals)
        return if (inAmt > 0.0) outAmt / inAmt else 0.0
    }
}
...
data class SwapResponse(
    ...
    fun getTransactionBytes(): ByteArray {
        return PlatformBase64.decode(swapTransaction)
    }
}
