...
class MwaWalletAdapter(
  ...
  override suspend fun signMessage(message: ByteArray, request: WalletRequest): ByteArray {
    ...
    return client.signTransactions(s, listOf(message)).firstOrNull()
      ?: throw IllegalStateException("Wallet returned no signed payloads for signMessage")
  }
...
private fun <T> chunk(list: List<T>, maxSize: Int): List<List<T>> {
  ...
  /** Close the MWA session and release all resources. */
  fun disconnect() {
