...
object SendPipeline {
  ...
  /** Thrown when simulation-first mode detects an error before send. */
  class SimulationFailedException(val reason: String) : RuntimeException("Simulation failed: $reason")
  ...
  data class Config(
    ...
    /** If true, simulate the transaction before sending and fail fast on errors. */
    val simulateFirst: Boolean = false
  )
  ...
  suspend fun <T> send(
    ...
    /** Optional: called to simulate the transaction before send. Return error string or null. */
    simulateTransaction: (suspend (txBytes: ByteArray) -> String?)? = null
  ): Result<T> {
    ...
    while (attempt < config.maxAttempts) {
      ...
      try {
        ...
        // Simulation-first: catch errors before submitting to the cluster
        if (config.simulateFirst && simulateTransaction != null) {
          val simErr = simulateTransaction(signed)
          if (simErr != null) throw SimulationFailedException(simErr)
        }
